    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-40 flex justify-between items-center px-4 w-full h-16 bg-surface-container/90 shadow-md backdrop-blur-md border-b border-outline-variant/30">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Botón Mi Lista Trigger */}
          <button
            type="button"
            onClick={onOpenDrawer}
            aria-label="Abrir Mi Lista de Destinos"
            className="flex items-center gap-2 bg-surface-container-high px-3.5 py-1.5 rounded-full border border-outline-variant/40 hover:bg-surface-container-highest transition-colors duration-150 active:scale-95 cursor-pointer"
          >
            ...
            {count > 0 && (
              <span className="bg-primary text-on-primary text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-sm">
                {count}
              </span>
            )}
          </button>
